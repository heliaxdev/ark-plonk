...
use crate::poseidon::{
    constants::PoseidonConstants, matrix::Matrix, mds::SparseMatrix,
    PoseidonError,
};
...
impl<'a, COM, S: PoseidonSpec<COM, WIDTH>, const WIDTH: usize> Clone
...
impl<'a, COM, S: PoseidonSpec<COM, WIDTH>, const WIDTH: usize>
    ...
    S: ?Sized,
{
    pub fn new(
        c: &mut COM,
        constants: &'a PoseidonConstants<S::ParameterField>,
    ) -> Self {
...
impl<F, P, const WIDTH: usize>
    PoseidonSpec<prelude::StandardComposer<F, P>, WIDTH> for PlonkSpec<WIDTH>
where
    F: PrimeField,
...
mod tests {
    ...
    #[test]
    // because poseidon_ref matches reference implementation, if optimized
    // poseidon matches poseidon_ref, then it also matches reference
    // implementation.
    ...
    fn check_plonk_spec_with_native() {
        ...
        let mut poseidon_circuit =
            Poseidon::<_, PlonkSpec<WIDTH>, WIDTH>::new(&mut c, &param);
