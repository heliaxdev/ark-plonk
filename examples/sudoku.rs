...
pub fn fiat_shamir_gadget<F: PrimeField, P: TEModelParameters<BaseField = F>>(
    composer: &mut StandardComposer<F, P>,
    secret: &[u8],
) -> Variable {
    // fiat-shamir using Poseidon hash with two inputs, so it works for a secret of at most 2*32=64 bytes
    ...
    let mut poseidon_circuit = Poseidon::<_, PlonkSpec<3>, 3>::new(
        composer,
        &poseidon_hash_param,
    );
    ...
    for i in 0..secret.len() / 32 {
        let f = F::from_le_bytes_mod_order(&secret[32 * i..min(32 * (i + 1), secret.len())]);
...
pub fn field_multiplication_gadget<F: PrimeField, P: TEModelParameters<BaseField = F>>(
    composer: &mut StandardComposer<F, P>,
    var_a: Variable,
    var_b: Variable,
) -> Variable {
    ...
    let var_zero = composer.zero_var();
    // Make first constraint a + b = c (as public input)
    composer.arithmetic_gate(|gate| gate.witness(var_a, var_b, Some(var_zero)).mul(one).pi(-c));
    composer.add_input(c)
}

// create the circuit corresponding to the last theorem of the PLONK paper, i.e. checking that
// prod([i + gamma for i in range(9)]) == prod([sigma[i] + gamma for i in range(9)]) for random gamma.
pub fn permutation_gadget<F: PrimeField, P: TEModelParameters<BaseField = F>>(
    composer: &mut StandardComposer<F, P>,
    permutation: [u8; 9],
    var_gamma: Variable,
) {
    // prod_i (i + gamma)
    let gamma = composer.get_value(&var_gamma);
    let mut lhs = F::one();
    for i in 0..9 {
        lhs *= F::from(i as u32) + gamma;
    }
    let var_lhs = composer.add_input(lhs);

    // prod_i (sigma(i) + gamma)
    let mut var_prod = composer.add_input(F::one());
    for i in 0..9 {
        // t = sigma(i) + gamma
        // prod = prod * t
        let var_sigma_i = composer.add_input(F::from(permutation[i]));
        let var_t = field_addition_gadget::<F, P>(composer, var_sigma_i, var_gamma);
        var_prod = field_multiplication_gadget::<F, P>(composer, var_prod, var_t);
    }
    composer.assert_equal(var_prod, var_lhs);
}
...
fn main() -> Result<(), Error> {
        ...
        P: TEModelParameters<BaseField = F>,
    {
        ...
        fn gadget(&mut self, composer: &mut StandardComposer<F, P>) -> Result<(), Error> {
            ...
            // entries of the sudoku are actually < 16. We can halve the size of the list representing the sudoku.
            let s1 = &sudoku[..sudoku.len()/2];
            let s2 = &sudoku[sudoku.len()/2..];
            let smaller_sudoku: Vec<u8> = s1.iter().zip(s2.iter()).map(|(b1,b2)| b1+16*b2).collect();
            // we generate gamma from this shorter list
            let var_gamma = fiat_shamir_gadget::<F, P>(composer, &smaller_sudoku);
            for row in self.sudoku {
                permutation_gadget::<F, P>(composer, row, var_gamma);
            }
            for i in 0..9 {
                let col: Vec<_> = self.sudoku.iter().map(move |row| row[i]).collect();
            ...
            for i in 0..3 {
                for j in 0..3 {
                    ...
                    let square: Vec<_> = sub_lines
                        ...
                        .collect();
                    let perm: [u8; 9] = square.concat()[0..9].try_into().unwrap();
    ...
    let time = Instant::now();
    let (proof, pi) = {
        let mut circuit: SudokuCircuit<Fr, JubjubParameters> =
            SudokuCircuit {
                sudoku: [
                    [7, 6, 0, 5, 3, 8, 1, 2, 4],
                    [2, 4, 3, 7, 1, 0, 6, 5, 8],
