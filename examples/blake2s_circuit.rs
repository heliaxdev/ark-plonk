        ...
        P: TEModelParameters<BaseField = F>,
    {
        ...
        fn gadget(
           ...
           let mod_32 = |composer : &mut StandardComposer<F,P>, a_var : Variable| {
               ...
               composer.range_gate(quotient_var, 32);
               return reminder_var;
           };
           ...
           | {
               ...
               if n % 2 == 0 {
                   ...
                   composer.constrain_to_constant(two, F::from(2 as u64), None);
           ...
           | {
               ...
               composer.range_gate(x_var, 32);
               composer.range_gate(y_var, 32);

               let _va1 = composer.arithmetic_gate(|gate| {
                   gate.witness(va, vb, None)
                       .add(F::one(), F::one())
                       .fan_in_3(F::one(),x_var)
               });
               let va1 = mod_32(composer, _va1);

               let temp1 = composer.xor_gate(vd, va1, 32);
               let vd1 = rotation(composer, temp1, 16, 32);
               ...
               let temp2 = composer.xor_gate(vb, vc1, 32);
               let vb1 = rotation(composer, temp2, 12, 32);

               let _va2 = composer.arithmetic_gate(|gate| {
                   gate.witness(va1, vb1, None)
                       .add(F::one(), F::one())
                       .fan_in_3(F::one(), y_var)
               });
               let va2 = mod_32(composer, _va2);

               let temp3 = composer.xor_gate(vd1, va2, 32);
               let vd2 = rotation(composer, temp3, 8, 32);
               ...
               let temp4 = composer.xor_gate(vb1, vc2, 32);
               let vb2 = rotation(composer, temp4, 7, 32);
